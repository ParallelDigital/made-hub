        ...
        <form id="class-form" action="{{ route('admin.classes.update', $class) }}" method="POST" class="px-6 py-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="time" name="start_time" id="start_time" value="{{ old('start_time', $class->start_time ? \Carbon\Carbon::parse($class->start_time)->format('H:i') : '') }}"
                           class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" required>
                    @error('start_time')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="time" name="end_time" id="end_time" value="{{ old('end_time', $class->end_time ? \Carbon\Carbon::parse($class->end_time)->format('H:i') : '') }}"
                           class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" required>
                    @error('end_time')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
            ...
            <div class="flex items-center">
                <input type="checkbox" name="members_only" id="members_only" value="1" {{ old('members_only', $class->members_only) ? 'checked' : '' }}
                       class="h-4 w-4 text-primary focus:ring-primary border-gray-600 bg-gray-700 rounded">
